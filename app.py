...
def _agent_call_gemini(message, context_page, settings, image_data=None, image_mime=None, model='gemini-2.5-flash', history=None):
...
def agent_chat():
    ...
    try:
        ...
        intent = _agent_call_gemini(message, context_page, settings, image_data, image_mime, model='gemini-3.1-pro-preview', history=history)
        itype  = intent.get('type', 'chat')
        model_used = '3.1 Pro'

        # Phase 2: ถ้าเป็น WRITE tool ให้ 3.1 Pro ตรวจสอบ params ให้รอบคอบขึ้น
        if itype == 'plan':
            pro_intent = _agent_call_gemini(message, context_page, settings, image_data, image_mime, model='gemini-3.1-pro-preview', history=history)
