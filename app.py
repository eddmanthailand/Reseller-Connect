...
def _bot_chat_reply(thread_id, reseller_id, user_message_text, conn):
    ...
    try:
        ...
        _FABRIC_KNOWLEDGE = """
=== ความรู้เรื่องผ้าและหลักสรีรศาสตร์การเลือกเสื้อผ้า (ใช้เป็นฐานความรู้) ===

[ประเภทผ้าและการเผื่อขนาด]
• ผ้าไม่ยืด (woven/non-stretch) เช่น วาเลนติโน่ ซาติน ผ้าฝ้าย ลินิน โพลีเอสเตอร์ทอ:
  - สะโพก: เผื่ออย่างน้อย 2"–3" (เพราะผ้าไม่มีความยืดหยุ่น ลุกนั่งจะดึงผ้า)
  - เอว: เผื่อ 0.5"–1" (ถ้าซิปหรือตะขอ ต้องพอดีหรือเล็กกว่าเล็กน้อย)
• ผ้ายืด (stretch/jersey/spandex):
  - สะโพก: เผื่อ 0.5"–1.5" ก็พอ เพราะผ้ายืดตามตัว
  - เอว: ยืดตามตัวได้ ไม่ต้องเผื่อมาก

[หลักสรีรศาสตร์การเคลื่อนไหว]
• เมื่อนั่ง: สะโพกขยายขึ้น 1"–1.5" จากท่ายืน → ผ้าไม่ยืดต้องเผื่อเพิ่ม
• เมื่อก้าว/เดิน: ต้องการช่องว่างที่ด้านหน้าขาพอก้าว ถ้ากระโปรงแคบเกินไปจะรั้ง
• กระโปรงทรงสอบ (A-line/pencil): ช่วงสะโพกสำคัญที่สุด ต้องเผื่อมากที่สุด

[เกณฑ์การเลือกไซส์ผ้าไม่ยืด (ใช้แทนเกณฑ์ทั่วไป)]
• สะโพก:
  ✗ ช่องว่าง < 2" → แน่นมาก นั่งไม่สบาย ลุกยาก
  ⚠ ช่องว่าง 2"–2.5" → แน่นพอใส่ได้ แต่ขยับตัวไม่สบาย ไม่แนะนำ
  ✓ ช่องว่าง 2.5"–4" → พอดีสบาย ขยับตัวและนั่งได้สบาย
  △ ช่องว่าง > 4" → หลวม ดูไม่ทรง
• เอว (ผ้าไม่ยืด): ช่องว่าง 0"–1" = พอดี; > 2" = หลวมเกิน; < 0" = ใส่ไม่ได้

[ตัวอย่างการคำนวณที่ถูกต้อง สำหรับผ้าไม่ยืด]
ลูกค้า: เอว 28" สะโพก 37"
ตาราง: S=เอว27/สะโพก36, M=เอว28/สะโพก37.5, L=เอว29/สะโพก39, XL=เอว30/สะโพก40.5
การคำนวณสะโพก:
  M: 37.5−37 = 0.5" < 2" → แน่นมาก นั่งไม่สบาย ❌
  L: 39−37 = 2" = พอดีขั้นต่ำ ⚠ นั่งจะแน่นนิดหน่อย
  XL: 40.5−37 = 3.5" → สบาย แต่อาจหลวมนิดหน่อย
→ สรุป: แนะนำ L (พอดีที่สุด) แต่ต้องแจ้งว่าถ้านั่งนานอาจแน่นนิดหน่อย เพราะผ้าไม่ยืด
"""
...
def _agent_call_gemini(message, context_page, settings, image_data=None, image_mime=None, model='gemini-2.5-flash'):
