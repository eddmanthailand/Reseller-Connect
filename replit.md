...
- **Modular Structure:** `routes/` (Stripe, AI Agent), `blueprints/` (Facebook Ads, Guest Bot, Member Bot, Warehouse, Marketing, Settings, Push/PWA, Mail Utils, Reseller, Orders, Products, Cart), `utils.py` (shared auth + csrf decorators + error handler), `database.py` (DB pool).
