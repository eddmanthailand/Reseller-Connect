...
- **Product Analytics Module (Mar 2026):** `blueprints/product_analytics.py` — tracks every product modal open in catalog (`/api/product-views/track`, no auth required); analytics endpoints (`/api/product-analytics/summary`, `/api/product-analytics/top-products`, `/api/product-analytics/daily-trend`, `/api/product-analytics/campaign-breakdown`). Table: `product_views` (product_id FK, session_id, utm_campaign, viewed_at). Dashboard: `static/js/dashboard-product-analytics.js` + page `#product-analytics` in admin dashboard. Tracking injected in `templates/catalog.html` `openSkuModal()` using `_trackMeta` (session_id, utm_campaign, utm_medium).
...
- **Auth Blueprint (modularized Mar 2026):** `blueprints/auth.py` — Google OAuth flow, Login/Register/Logout, Password Reset, Session management, Page routes (dashboard, admin, reseller pages). `oauth = OAuth()` initialized via `oauth.init_app(app)` in app.py. `@app.after_request` cache-control header kept in app.py.
- **Products Blueprint (modularized Mar 2026):** `blueprints/products.py` — Brand CRUD, Category CRUD, Product Management (SPU/SKU/images/options), Product Customization & Tier Pricing (routes `/api/brands`, `/api/categories`, `/api/products/*`, `/api/customizations/*`).
- **Cart Blueprint (modularized Mar 2026):** `blueprints/cart.py` — Shopping cart add/update/remove/checkout (routes `/api/cart/*`).
- **Reseller Blueprint (modularized Mar 2026):** `blueprints/reseller.py` — Reseller Dashboard, Thailand Address, Customers, Product Catalog, Cart, MTO API, Reseller Pages (routes `/api/reseller/*`, `/api/mto/*`).
- **Orders Blueprint (modularized Mar 2026):** `blueprints/orders.py` — Order API, Admin Dashboard Statistics, Admin Order Management, MTO Admin (routes `/api/orders`, `/api/admin/orders/*`, `/api/admin/dashboard-stats`, `/api/admin/mto/*`). Also contains MTO email helper functions.
- **Mail Utils (modularized Mar 2026):** `blueprints/mail_utils.py` — `send_email`, `send_order_status_chat`, `send_order_notification_to_admin`, `send_order_status_email`, `send_low_stock_alert`, `send_password_reset_email`, `log_activity`.
...
- **Public Retail Storefront (B2C):** `/shop` guest checkout (no signup) for nursing students. Routes in `blueprints/shop.py`. Guest orders attach to a dedicated retail system user with `order_type='retail'` + high-entropy `guest_token`. Bronze-tier pricing shown as full+discount; shows all active products, no minimum. Reuses `shipping_weight_rates` via `_calc_shipping_fee` (match-or-zero, no heaviest-bracket fallback). PromptPay QR + slip Thunder auto-verify (slip count capped per order). Guest order tracking at `/shop/track` via order_number+email. Auto email tracking link on ship (`admin_ship_order` in orders.py). Admin sees pink "ลูกค้าขายปลีก" recipient card + retail badge + "🧾 พิมพ์ใบส่งสินค้า" delivery note (items+prices, "ไม่ใช่ใบกำกับภาษี") in `dashboard-orders.js`. Separate FB Pixel retail events (`channel:retail`). Templates: `templates/shop.html`, `templates/shop_track.html`; JS: `static/js/shop.js`.
...
- **Modular Structure:** `routes/` (Stripe, AI Agent), `blueprints/` (Auth, Facebook Ads, Guest Bot, Member Bot, Warehouse, Marketing, Settings, Push/PWA, Mail Utils, Reseller, Orders, Products, Cart), `utils.py` (shared auth + csrf decorators + error handler), `database.py` (DB pool). `app.py` is now only ~420 lines (config + scheduler + blueprint registration).
