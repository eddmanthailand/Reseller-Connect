...
- **Marketing Module:** Admin CRUD for promotions (auto-applied, tier/brand/category conditions, stackable flag) and coupons (assign to users, min spend, max discount, expiry, usage limits). Reseller UI: coupon input at checkout with preview-discount API, coupon wallet picker modal (bottom sheet), coupon wallet section in Profile page. Layer system: Tier pricing → Best auto-promotion → Coupon (if stackable or no promo). `create_order` applies coupon+promotion and marks coupon as used.
