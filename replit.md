...
- **Quick Order (ขายด่วน) — Enhanced + Label OCR + Shipping Address:** `orders` table has `is_quick_order BOOLEAN` + `platform VARCHAR(30)` + `shipping_name/shipping_phone/shipping_address/shipping_province/shipping_district/shipping_subdistrict/shipping_postal` columns. **Label OCR**: ปุ่ม "📷 อ่านใบปะหน้าอัตโนมัติ" ในฟอร์ม — อัปโหลดรูปใบปะหน้า Shopee/Lazada → `POST /api/admin/quick-order/parse-label` ส่งให้ Gemini Vision → ดึง platform/tracking/ชื่อ/เบอร์/ที่อยู่ → แสดง card ยืนยัน → กด "ใช้ข้อมูลนี้" auto-fill ฟอร์ม รวมถึงช่องที่อยู่จัดส่ง; parse-label ตรวจสอบเบอร์โทรในระบบก่อน return `customer_status` (existing/new) + `existing_customer`; ต้องการ `GEMINI_API_KEY`; package `google-genai`; model `gemini-2.5-flash`. **Phone lookup real-time**: พิมพ์เบอร์ในฟอร์ม quick order → debounce 0.6s → ค้นหาในระบบ → แสดง badge "✓ ลูกค้าเก่า" หรือ "+ ลูกค้าใหม่" + auto-fill ชื่อถ้าพบ. **Shipping address on order** (ไม่เขียนทับ customer profile): address จากใบปะหน้าบันทึกลง orders.shipping_* ไม่แตะ customers.address — ลูกค้าคนเดียวสั่งหลายที่ได้โดยไม่ปะปน. **Auto-save customer**: เมื่อสร้าง quick order ที่มีเบอร์ → upsert customers table (ON CONFLICT phone); ถ้าไม่มีเบอร์แต่มีชื่อ → INSERT ใหม่; return `customer_status` ใน response. Quick orders created via `POST /api/admin/quick-order`; Orders page has mode toggle: "ออเดอร์ตัวแทน" vs "⚡ ขายด่วน". Quick Orders tab shows platform badge (colored), tracking link, status tabs, action buttons. Status update via `POST /api/admin/quick-orders/<id>/status`; stock restoration via `POST /api/admin/quick-orders/<id>/restore-stock`. iShip webhook: `returned` status updates order+shipment to 'returned'. Regular orders API excludes quick orders (`is_quick_order = FALSE`).
